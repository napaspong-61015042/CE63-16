...
#define FIREBASE_HOST "https://motorcycle-theft.firebaseio.com/"
#define FIREBASE_AUTH "6Ww8PReofVxLpFuUgHLkXP4piIhqrlsaMYlfnVIk"
...
void onReceiveData(const uint8_t *mac, const uint8_t *Indata, int len) {
  ...
  char text[100];
  dataReceive.toCharArray(text, 100);
  
  if (macAddressId.length() < 17) {
    for (int i = 1; i <= 17; i++) {
      macAddressId += text[i];
    }
  }
  
  Serial.println(macAddressId);
...
void loop() {
  ...
  if (millis() - sendDataPrevMillis > 15000)
  {
    ...
    if (Firebase.setString(firebaseData, path + "/MacAdress_BASE Station", macAddressId))
    {
...
//    if (Firebase.setString(firebaseData, path + "/STATUS", dataReceive))
//    {
//      Serial.println("PASSED");
//      Serial.println("PATH: " + firebaseData.dataPath());
//      Serial.println("TYPE: " + firebaseData.dataType());
